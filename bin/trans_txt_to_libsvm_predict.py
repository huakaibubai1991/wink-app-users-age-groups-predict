...
def txt_to_libsvm(input_data_path, libsvm_data_path):
    ...
    func = lambda col: 'float32' if col.startswith('v') else 'int64' if col == 'gid' else 'int8'
    v_columns = [f'v{i}' for i in range(1, 2181)]
    columns = ["gid"] + v_columns
    ...
    gid = input_data["gid"]
    #gid_int = [str(g) for g in gid]
    #gid_str = gid.astype(str)
    gid_int = gid.astype(np.int64)
    ...
    dump_svmlight_file(features_array, gid_int, libsvm_data_path, zero_based=False)
